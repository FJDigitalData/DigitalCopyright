...
public class SysProfileController extends BaseController {
     ...
     * 头像上传
     */
    @Operation(summary = "修改当前登录用户个人头像", parameters = {
            @Parameter(name = "file", description = "头像文件", required = true, in = ParameterIn.QUERY, schema = @Schema(type = "file"))
    })
    @Log(title = "用户头像", businessType = BusinessType.UPDATE)
    @PostMapping("/avatar")
    public AjaxResult avatar(@RequestParam MultipartFile file) throws Exception {
        if (!file.isEmpty()) {
            LoginUser loginUser = getLoginUser();
