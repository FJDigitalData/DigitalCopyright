package com.ruoyi.system.amazon;

import cn.hutool.core.lang.Assert;
import cn.hutool.core.util.StrUtil;
import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import com.ruoyi.amazon.spapi.SellingPartnerAPIAA.*;
import com.ruoyi.amazon.spapi.api.*;
import com.ruoyi.amazon.spapi.client.ApiClient;
import com.ruoyi.amazon.spapi.client.ApiException;
import com.ruoyi.amazon.spapi.model.tokens.CreateRestrictedDataTokenRequest;
import com.ruoyi.amazon.spapi.model.tokens.CreateRestrictedDataTokenResponse;
import com.ruoyi.amazon.spapi.model.tokens.RestrictedResource;
...
public class ApiBuildService implements InitializingBean {
    ...
    public AWSAuthenticationCredentials getAwsAuthenticationCredentials(AmazonAuthority auth) {
        if (StrUtil.isNotBlank(auth.getAccessKeyId())) {
    ...
    public AWSAuthenticationCredentialsProvider getAWSAuthenticationCredentialsProvider(AmazonAuthority auth) {
        if (StrUtil.isNotBlank(auth.getRoleArn())) {
    ...
    public LWAAuthorizationCredentials getLWAAuthorizationCredentials(AmazonAuthority auth) {
        if (StrUtil.isNotBlank(auth.getClientId())) {
    ...
    private String getEndPoint(String region) {
        region = StrUtil.isNotBlank(region) ? region.trim() : region;
        String endpoint = "";
        sandbox = "false";
    ...
    private void initClient(ApiClient client) {
        ...
        if (needproxy != null && "true".equals(needproxy)) {
            SocketAddress address = new InetSocketAddress("127.0.0.1", 1080);
            Proxy proxy = new Proxy(Proxy.Type.SOCKS, address);
            client.setHttpClient(client.getHttpClient().newBuilder().proxy(proxy).build());
        }
    }

    public static OkHttpClient getInitClient() {
    ...
    public Response buildAndExecuteRestrictedRequest(AmazonAuthority auth, RestrictedResource resource, String restrictedDataToken, RequestBody requestBody) throws IOException {
        ...
        OkHttpClient okHttpClient = getInitClient();
